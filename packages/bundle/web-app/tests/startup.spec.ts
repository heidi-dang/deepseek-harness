...
describe('web command-line provider', () => {
  it('publishes each flag and releases direct service expressions', async () => {
    const { values, observed } = await bootProvider([
      ...
      '--trusted-config-host', 'box.ts.net',
    ])
    expect(values).toEqual({
      ...
      privilegedTrustedHosts: ['box.ts.net'],
    })
  ...
  it('leaves deployment values to each consumer when flags omit them', async () => {
    const { values, observed } = await bootProvider([])
    expect(values).toEqual({ openBrowser: true, trustedHosts: [], privilegedTrustedHosts: [] })
  ...
  it('prints its own help and leaves the consumer pending', async () => {
    ...
    expect(observed.out).toContain('--trusted-config-host')
  ...
  it('accepts the all-interfaces host as the explicit remote-access mode', async () => {
