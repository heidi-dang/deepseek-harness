...
describe('ThemeRuntime', () => {
  it('defaults to the system preference resolved against prefers-color-scheme', () => {
    ...
    expect(snapshot.themes.map(t => t.id)).toEqual(['light', 'dark', 'night-owl'])
  })
  ...
  it('registered themes join the snapshot; disposing the active one resets to default', () => {
    ...
    expect(theme.getTheme().themes.map(t => t.id)).toEqual(['light', 'dark', 'night-owl', 'sepia'])
    theme.setTheme('sepia')
    expect(theme.getTheme().active.tokens['--dsw-alias-bg-base']).toBe('red')
    dispose()
    expect(theme.getTheme().preference).toBe('system')
    expect(theme.getTheme().themes.map(t => t.id)).toEqual(['light', 'dark', 'night-owl'])
